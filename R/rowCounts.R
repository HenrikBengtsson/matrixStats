...
rowCounts <- function(x, rows=NULL, cols=NULL, value=TRUE, na.rm=FALSE, dim.=dim(x), ...) {
  ...
  if (is.numeric(x) || is.logical(x)) {
    ...
    hasNAs <- TRUE
    counts <- .Call("rowCounts", x, dim., rows, cols, value, 2L, na.rm, hasNAs)
  } else {
...
colCounts <- function(x, rows=NULL, cols=NULL, value=TRUE, na.rm=FALSE, dim.=dim(x), ...) {
  ...
  if (is.numeric(x) || is.logical(x)) {
    na.rm <- as.logical(na.rm)
    hasNAs <- TRUE
    counts <- .Call("colCounts", x, dim., rows, cols, value, 2L, na.rm, hasNAs)
  } else {
    if (is.vector(x)) dim(x) <- dim.
...
count <- function(x, idxs=NULL, value=TRUE, na.rm=FALSE, ...) {
  ...
  if (is.numeric(x) || is.logical(x)) {
    na.rm <- as.logical(na.rm)
    hasNAs <- TRUE
    counts <- .Call("count", x, idxs, value, 2L, na.rm, hasNAs)
  } else {
    # Apply subset
...
rowAlls <- function(x, rows=NULL, cols=NULL, value=TRUE, na.rm=FALSE, dim.=dim(x), ...) {
  if (is.numeric(x) || is.logical(x)) {
    na.rm <- as.logical(na.rm)
    hasNAs <- TRUE
    counts <- .Call("rowCounts", x, dim., rows, cols, value, 0L, na.rm, hasNAs)
    as.logical(counts)
  } else {
    if (is.vector(x)) dim(x) <- dim.
...
colAlls <- function(x, rows=NULL, cols=NULL, value=TRUE, na.rm=FALSE, dim.=dim(x), ...) {
  if (is.numeric(x) || is.logical(x)) {
    na.rm <- as.logical(na.rm)
    hasNAs <- TRUE
    counts <- .Call("colCounts", x, dim., rows, cols, value, 0L, na.rm, hasNAs)
    as.logical(counts)
  } else {
    if (is.vector(x)) dim(x) <- dim.
...
allValue <- function(x, idxs=NULL, value=TRUE, na.rm=FALSE, ...) {
  if (is.numeric(x) || is.logical(x)) {
    na.rm <- as.logical(na.rm)
    hasNAs <- TRUE
    counts <- .Call("count", x, idxs, value, 0L, na.rm, hasNAs)
    as.logical(counts)
  } else {
    # Apply subset
    if (!is.null(idxs)) x <- x[idxs]
...
rowAnys <- function(x, rows=NULL, cols=NULL, value=TRUE, na.rm=FALSE, dim.=dim(x), ...) {
  if (is.numeric(x) || is.logical(x)) {
    na.rm <- as.logical(na.rm)
    hasNAs <- TRUE
    counts <- .Call("rowCounts", x, dim., rows, cols, value, 1L, na.rm, hasNAs)
    as.logical(counts)
  } else {
    if (is.vector(x)) dim(x) <- dim.
...
colAnys <- function(x, rows=NULL, cols=NULL, value=TRUE, na.rm=FALSE, dim.=dim(x), ...) {
  if (is.numeric(x) || is.logical(x)) {
    na.rm <- as.logical(na.rm)
    hasNAs <- TRUE
    counts <- .Call("colCounts", x, dim., rows, cols, value, 1L, na.rm, hasNAs)
    as.logical(counts)
  } else {
    if (is.vector(x)) dim(x) <- dim.
...
anyValue <- function(x, idxs=NULL, value=TRUE, na.rm=FALSE, ...) {
  if (is.numeric(x) || is.logical(x)) {
    na.rm <- as.logical(na.rm)
    hasNAs <- TRUE
    counts <- .Call("count", x, idxs, value, 1L, na.rm, hasNAs)
    as.logical(counts)
  } else {
    # Apply subset
    if (!is.null(idxs)) x <- x[idxs]
