...
for (rows in indexCases) {
  for (cols in indexCases) {
    ...
    validateIndicesTestMatrix(x, rows, cols, ftest=rowOrderStats, fsure=rowOrderStats_R, which=which, probs=probs, mc.cores=2L)
    validateIndicesTestMatrix(x, rows, cols, fcolTest=colOrderStats, fsure=rowOrderStats_R, which=which, probs=probs, mc.cores=2L)
  }
}
