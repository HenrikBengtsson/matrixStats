...
for (trim in c(0.1, 0.4, 0.5)) {
  ...
  for (mode in c("integer", "logical", "double")) {
    ...
    y0 <- apply(x, 1, mean, trim = trim, na.rm = FALSE)
    y1 <- rowMeans2(x, trim = trim, na.rm = FALSE)
    stopifnot(all.equal(y1, y0))
    
    y0 <- apply(x, 2, mean, trim = trim, na.rm = FALSE)
    y1 <- colMeans2(x, trim = trim, na.rm = FALSE)
    stopifnot(all.equal(y1, y0))
  }
  ...
  for (mode in c("integer", "logical", "double")) {
    ...
    y0 <- apply(x, 1, mean, trim = trim, na.rm = TRUE)
    y1 <- rowMeans2(x, na.rm = TRUE)
    stopifnot(all.equal(y1, y0))
    
    y0 <- apply(x, 2, mean, trim = trim, na.rm = TRUE)
  ...
  for (kk in seq_len(n_sims)) {
    ...
    for (mode in modes) {
      ...
      # rowMeans2():
      y0 <- apply(x, 1, mean, trim = trim, na.rm = na.rm)
      y1 <- rowMeans2(x, trim = trim, na.rm = na.rm)
      stopifnot(all.equal(y1, y0))
      y2 <- colMeans2(t(x), trim = trim, na.rm = na.rm)
      stopifnot(all.equal(y2, y0))
      
      # colMeans2():
      y0 <- apply(x, 2, mean, trim = trim, na.rm = na.rm)
      y1 <- colMeans2(x, trim = trim, na.rm = na.rm)
      stopifnot(all.equal(y1, y0))
      y2 <- rowMeans2(t(x), trim = trim, na.rm = na.rm)
      stopifnot(all.equal(y2, y0))
    }
  } # for (kk ...)
} # for (trim...)