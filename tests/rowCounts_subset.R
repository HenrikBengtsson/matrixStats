...
for (rows in indexCases) {
  for (cols in indexCases) {
    ...
    validateIndicesTestMatrix(x, rows, cols, fcolTest = colCounts, fsure = rowCounts_R, value = 0, na.rm = TRUE)
...
for (rows in indexCases) {
  for (cols in indexCases) {
    ...
    validateIndicesTestMatrix(x, rows, cols, fcolTest = colCounts, fsure = rowCounts_R, value = "g", na.rm = TRUE)
