...
SEXP rowCumprods(SEXP x, SEXP dim, SEXP rows, SEXP cols, SEXP byRow, SEXP cores) {
  int byrow, cores2;
  SEXP ans = NILSXP;
  R_xlen_t nrow, ncol;
  ...
  if (isReal(x)) {
    ...
    rowCumprods_Real[rowsType][colsType](REAL(x), nrow, ncol, crows, nrows, ccols, ncols, byrow, REAL(ans), nrows, oks, cores2);
    UNPROTECT(1);
  } else if (isInteger(x)) {
    PROTECT(ans = allocMatrix(INTSXP, nrows, ncols));
    if (byrow) oks = (int*)R_alloc(nrows, sizeof(int));
    rowCumprods_Integer[rowsType][colsType](INTEGER(x), nrow, ncol, crows, nrows, ccols, ncols, byrow, INTEGER(ans), nrows, oks, cores2);
    UNPROTECT(1);
  }
