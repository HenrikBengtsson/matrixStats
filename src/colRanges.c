...
SEXP colRanges(SEXP x, SEXP dim, SEXP rows, SEXP cols, SEXP what, SEXP naRm, SEXP hasNA, SEXP cores) {
  ...
  if (isReal(x)) {
    ...
    colRanges_Real[rowsType][colsType](REAL(x), nrow, ncol, crows, nrows, ccols, ncols, what2, narm, hasna, REAL(ans), ncols, is_counted, cores2);
    UNPROTECT(1);
  } else if (isInteger(x)) {
    if (what2 == 2) {
    ...
    colRanges_Integer[rowsType][colsType](INTEGER(x), nrow, ncol, crows, nrows, ccols, ncols, what2, narm, hasna, INTEGER(ans), ncols, is_counted, cores2);
